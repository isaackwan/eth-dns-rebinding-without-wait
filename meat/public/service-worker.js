...
// source: https://stackoverflow.com/a/38235477/1348400
var ESC_MAP = {
    '&': '&amp;',
    '<': '&lt;',
    '>': '&gt;',
...
self.addEventListener("sync", function(event) {
	if (event.tag === "try-localhost2") {
		const promise = (async function() {
			console.debug("Hello!", new Date());
			...
			console.debug("Response from SW", resp);
			...
			tx.objectStore('objs').put({
				time: new Date(),
				data: escapeHTML(result),
			});
			await tx.complete;

			if (!(resp.status >= 200 && resp.status < 300)) { // https://github.com/github/fetch/issues/155#issuecomment-108288863
				return Promise.reject(new Error("response is not 2XX" + resp));
			}

			console.log("Finished hack");
		})();
